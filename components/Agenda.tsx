...
import { CancelIcon, CheckIcon, CopyIcon, EditIcon, GripVerticalIcon, MarkdownIcon, PauseIcon, PlayIcon, PlusIcon, TrashIcon } from './icons';
...
const SortableActivityItem: React.FC<SortableActivityItemProps> = ({ activity, onStart, onDelete, onDuplicate, onEdit, isActive }) => {
    const isHeading = activity.type.startsWith('h');
    ...
    if (isHeading) {
        const HeadingTag = activity.type as 'h1' | 'h2' | 'h3';
        const headingClasses = {
            h1: 'text-2xl font-bold text-slate-100 my-4 border-b border-slate-700 pb-2',
            h2: 'text-xl font-semibold text-slate-200 my-3',
            h3: 'text-lg font-medium text-slate-300 my-2',
        };
        return (
            <div ref={setNodeRef} style={style} className={`px-3 ${headingClasses[HeadingTag]}`}>
                {activity.name}
            </div>
        );
    }
...
export const Agenda: React.FC<AgendaProps> = ({ activities, setActivities, activeActivityId, onStartActivity, onDeleteActivity, onDuplicateActivity, onEditActivity, onManualUpdateActivity }) => {
  ...
  const handleSaveMarkdown = (newActivities: Activity[]) => {
  ...
  return (
    <div className="bg-slate-800/50 p-6 lg:p-8 rounded-2xl flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-slate-300">Agenda della Riunione</h2>
        <button 
          onClick={() => setIsMarkdownEditing(!isMarkdownEditing)}
          className="flex items-center gap-2 text-sm bg-slate-700 hover:bg-slate-600 text-slate-300 font-semibold py-2 px-3 rounded-md transition-colors"
          aria-label={isMarkdownEditing ? "Torna alla vista standard" : "Modifica con Markdown"}
        >
          <MarkdownIcon className="w-4 h-4" />
          <span>{isMarkdownEditing ? 'Vista Standard' : 'Modifica Rapida'}</span>
        </button>
      </div>
      ...
      {isMarkdownEditing ? (
        <MarkdownEditor 
          ...
          onCancel={() => setIsMarkdownEditing(false)}
        />
      ) : (
        <>
